...
# ==============================================================================
# BTC SNIPER 5M - STRICT 5M CANDLE-CLOSE ANCHORED ENGINE (ZERO SPAM)
# ==============================================================================
...
<body>
    ...
    <div class="workspace">
        ...
        <div class="bottom-bar">
            ...
            <div class="metric-cell">
                ...
                <div class="cell-body" id="val-setup" style="color:#38bdf8;">MONITORING SQUEEZE</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let candles = [];
        let activeTrade = JSON.parse(localStorage.getItem('sniper_active_trade_v3') || 'null');
        let tradeHistory = JSON.parse(localStorage.getItem('sniper_history_v3') || '[]');
        let evaluatedCandleTime = 0;
        ...
        function updateScreenDisplays() {{
            ...
            if (activeTrade) {{
                ...
                document.getElementById('val-setup').innerText = "MONITORING SQUEEZE";
        ...
        function clearHistoryData() {{
            tradeHistory = [];
            localStorage.removeItem('sniper_history_v3');
            activeTrade = null;
            localStorage.removeItem('sniper_active_trade_v3');
            removeTradeLines();
            markersList = [];
            series.setMarkers([]);
        ...
        // MANAGE ACTIVE TRADE (TICK BY TICK)
        function checkTradeProgression(livePrice) {{
            if (!activeTrade) return;

            if (activeTrade.type === "LONG") {{
                // Target 1 Hit
                if (!activeTrade.tp1_hit && livePrice >= activeTrade.tp1) {{
                    activeTrade.tp1_hit = true;
                    activeTrade.sl = +(activeTrade.entry + 15).toFixed(1);
                    localStorage.setItem('sniper_active_trade_v3', JSON.stringify(activeTrade));
                    drawTradeLinesOnChart();
                    sendTelegram(`🎯 BTC LONG TP1 HIT (+90 pts) at $${{activeTrade.tp1}}\\nSL locked at Breakeven: $${{activeTrade.sl}}`);
                    updateScreenDisplays();
                }}
                // Runner Hit
                if (livePrice >= activeTrade.tp2) {{
                    sendTelegram(`🚀 BTC LONG RUNNER REACHED (+${{activeTrade.reward}} pts)!`);
                    recordTradeHistory("LONG", activeTrade.entry, "TP RUNNER", `+${{activeTrade.reward}}`);
                }} else if (livePrice <= activeTrade.sl) {{
                    ...
                    recordTradeHistory("LONG", activeTrade.entry, res, pts);
                }}
                ...
                if (livePrice <= activeTrade.tp2) {{
                    ...
                    recordTradeHistory("SHORT", activeTrade.entry, "TP RUNNER", `+${{activeTrade.reward}}`);
                }} else if (livePrice >= activeTrade.sl) {{
                    let res = activeTrade.tp1_hit ? "BE LOCKED" : "SL HIT";
                    let pts = activeTrade.tp1_hit ? "+15" : `-${{activeTrade.risk}}`;
                    sendTelegram(`🛡️ BTC SHORT ${{res}} at $${{activeTrade.sl}}`);
                    recordTradeHistory("SHORT", activeTrade.entry, res, pts);
                }}
            }}
        }}

        // STRICT EVALUATION ONLY ON 5M CLOSED CANDLES
        function evaluateClosedCandle(closedBar) {{
            if (activeTrade) return;
            if (closedBar.time <= evaluatedCandleTime) return;
            evaluatedCandleTime = closedBar.time;

            if (candles.length < 15) return;
            const lookback = candles.slice(-8, -1);
            const highRange = Math.max(...lookback.map(c => c.high));
            const lowRange = Math.min(...lookback.map(c => c.low));
            const body = closedBar.close - closedBar.open;

            // Institutional Criteria: Breakout of 40m range with decisive body close
            const isLong = (closedBar.close > highRange) && (body >= 25.0);
            const isShort = (closedBar.close < lowRange) && (body <= -25.0);

            if (isLong) {{
                let entry = closedBar.close;
                let sl = +(Math.min(closedBar.low, lowRange) - 25.0).toFixed(1);
                let risk = +(entry - sl).toFixed(1);
                if (risk < 85) {{ risk = 95; sl = +(entry - 95).toFixed(1); }}
                if (risk > 160) {{ risk = 140; sl = +(entry - 140).toFixed(1); }}
                ...
                activeTrade = {{
                    ...
                    risk: risk, reward: reward, tp1_hit: false
                }};
                localStorage.setItem('sniper_active_trade_v3', JSON.stringify(activeTrade));
                ...
                markersList.push({{
                    ...
                    color: '#00e676',
                    shape: 'arrowUp',
                    text: 'BUY @ $' + entry.toFixed(1)
                }});
                series.setMarkers(markersList);

                playBeep();
                updateScreenDisplays();
                sendTelegram(`⚡ BTC CONFIRMED 5M LONG\\n\\n📍 Entry: $${{entry.toFixed(1)}}\\n🛡️ SL: $${{sl.toFixed(1)}} (-$${{risk}})\\n🎯 TP 1: $${{tp1}} (+90 pts BE)\\n🚀 TP 2: $${{tp2}} (+${{reward}} pts)\\n\\nSetup: Closed Breakout Confirmed`);
            }}
            else if (isShort) {{
                let entry = closedBar.close;
                let sl = +(Math.max(closedBar.high, highRange) + 25.0).toFixed(1);
                ...
                markersList.push({{
                    ...
                    color: '#ff3b30',
                    shape: 'arrowDown',
                    text: 'SELL @ $' + entry.toFixed(1)
                }});
                series.setMarkers(markersList);

                playBeep();
                updateScreenDisplays();
                sendTelegram(`⚡ BTC CONFIRMED 5M SHORT\\n\\n📍 Entry: $${{entry.toFixed(1)}}\\n🛡️ SL: $${{sl.toFixed(1)}} (-$${{risk}})\\n🎯 TP 1: $${{tp1}} (+90 pts BE)\\n🩸 TP 2: $${{tp2}} (+${{reward}} pts)\\n\\nSetup: Closed Breakdown Confirmed`);
            }}
        }}

        function recordTradeHistory(type, entry, res, pts) {{
            ...
            localStorage.setItem('sniper_history_v3', JSON.stringify(tradeHistory));
            activeTrade = null;
            localStorage.removeItem('sniper_active_trade_v3');
            removeTradeLines();
            updateScreenDisplays();
        ...
        function updateLiveCandle(price, rawTimeSec) {{
            ...
            if (barTime === last.time) {{
                ...
                // Previous candle has officially CLOSED
                evaluateClosedCandle(last);
